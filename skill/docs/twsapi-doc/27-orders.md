...
[Order Class Reference](../undefined/index.md)
...
Immediately after an order is submitted correctly, the TWS will start sending events concerning the order’s activity via [EWrapper.openOrder](../undefined/index.md) and [EWrapper.orderStatus](../undefined/index.md)

Advisors executing allocation orders will receive execution details and commissions for the allocation order itself. To receive allocation details and commissions for a specific subaccount [EClient.reqExecutions](../undefined/index.md) can be used.
...
Consider using [EWrapper.NextValidId](../undefined/index.md)
...
In the event a user would like to designate a trade to take place during the Overnight trading hours, the [Order object](../undefined/index.md) must set “includeOvernight” set to True and optionally set the Exchange value to OVERNIGHT.
...
When placing orders via the API and building a robust trading system, it is important to monitor for callback notifications, specifically for **[IBApi::EWrapper::error](../undefined/index.md)**, [**IBApi:**:**EWrapper::orderStatus**](../undefined/index.md) changes, [**IBApi::EWrapper::openOrder**](../undefined/index.md) warnings, and **[IBApi::EWrapper::execDetails](../undefined/index.md)** to ensure proper operation.

If you experience issues with orders you place via the API, such as orders not filling, the first thing to check is what these callbacks returned. Your order may have been rejected or cancelled. If needed, see the **[API Log](../undefined/index.md)** section, for information on obtaining your API logs or submitting them for review.

Common cases of order rejections, cancellations, and warnings, and the corresponding message returned:

*   If an order is subject to a large size (LGSZ) reject, the API client would receive **Error (201)** via **[IBApi::EWrapper::error](../undefined/index.md)**. The error text would indicate that order size too large and suggest another smaller size.
...
*   If an order is subject to price checks the client may receive status (cancelled) + **Error (202)** via [**IBApi.EWrapper.orderStatus**](../undefined/index.md) and **[IBApi::EWrapper::error](../undefined/index.md)**. The error text would indicate the price is too far from current price.
...
*   The client may receive warning Text via **[IBApi::EWrapper::openOrder](../undefined/index.md)** indicating that the order could be subject to price capping.
    ...
    *   **_[mktCapPrice](../undefined/index.md)_** – If an order has been capped, this indicates the current capped price (returned to [**IBApi.EWrapper.orderStatus**](../undefined/index.md))
...
Apart from InitMarginChange, there are other supported variables. For details, please visit: [https://www.interactivebrokers.com/campus/ibkr-api-page/twsapi-ref/#orderstate-ref](../undefined/index.md)
