  ...
  [20-delayed-market-data.md](20 Market Data: Delayed) -> 

 20 Market Data: Delayed
...
The API can request Live, Frozen, Delayed and Delayed Frozen market data from Trader Workstation by switching market data type via the [EClient.reqMarketDataType](20.02-request-md-type) before making a market data request. A successful switch to a different (non-live) market data type for a particular market data request will be indicated by a callback to [EWrapper.marketDataType](20.03-receive-md-type) with the ticker ID of the market data request which is returning a different type of data.

*   A [EClient.reqMarketDataType](20.02-request-md-type) callback of **1** will occur automatically after invoking reqMktData if the user has live data permissions for the instrument.
